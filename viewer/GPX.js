    ...
    */
    ...
    read: function(doc) {
        ...
        for (var i=0, len=tracks.length; i<len; i++) {
            var segs = this.getElementsByTagNameNS(tracks[i], tracks[i].namespaceURI, "trkseg");
            for (var j = 0, seglen = segs.length; j < seglen; j++) {
                var points = this.getElementsByTagNameNS(segs[j], segs[j].namespaceURI, "trkpt");
                var center, point, desc, time, name, accuracy, confidence;
                for (var i = 0, len = points.length; i < len; i++) {
                    desc = points[i].getElementsByTagName("desc")[0].firstChild.nodeValue.split(",");
